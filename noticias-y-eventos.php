...
if($totalRows>0)
{
	...
	for($i=0; $i<$numRows; $i++)
	{
		...
		if($module['fecha']=='1')
		{
			...
			$info[$i]['date'] = date('F j, Y', mktime(0, 0, 0, intval($date[1]), intval($date[2]), intval($date[0])));
			if($_SESSION['lang']=='1')
				$info[$i]['date'] = date_to_spanish($info[$i]['date']);
		}
		...
		{ ?>
			<div class="newsItem">
				<div class="niHPic"><?
					if($imgInfo[$i])  // El registro tiene por lo menos una imagen asociada
					{?>
						<a href="<?= $module['archivo'].'-single.php?id='.$info[$i]['id']; ?>"><img src="<?= 'uploads/news/'.$info[$i]['id'].'/'.$imgFile[$i][1] ?>" alt="<?= $imgInfo[$i]['title'];?>"></a><?
					} ?>				
				</div>
				<h2><a href="<?= $module['archivo'].'-single.php?id='.$info[$i]['id']; ?>"><?= $info[$i]['title']; ?></a></h2>
				<div class="hiDate"><?= $info[$i]['date'] ?></div>
				<div class="niSummary">
					<?= $info[$i]['summary']; ?>
				</div>
				<a href="<?= $module['archivo'].'-single.php?id='.$info[$i]['id']; ?>" class="hiMore"><?= VER_MAS ?> &raquo;</a>
				<div class="clear"></div>
			</div><?
		}?>
		<div class="clear"></div><?
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.newsItem:odd').addClass('noMarginR');
	});
</script>
<? include("footer.php"); ?>