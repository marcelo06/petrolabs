...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	if($changed)
	{
		...
		file_put_contents("lib/database.txt", "$_POST[host] $_POST[user] $_POST[password] $_POST[database]");
...
$f = file_get_contents("lib/database.txt");
$database = explode(" ", $f); ?>
<header>
...
<div class="contentPane">
	<form id="setDbForm" name="setDbForm" action="<?= $_SERVER['PHP_SELF']; ?>" method="post">
		<input type="hidden" name="old_host" value="<?= $database[0]; ?>" />
		<input type="hidden" name="old_user" value="<?= $database[1]; ?>" />
		<input type="hidden" name="old_password" value="<?= $database[2]; ?>" />
		<input type="hidden" name="old_database" value="<?= $database[3]; ?>" />
		<table class="formTable">
			<tr>
				<th>Host:</th>
				<td><input type="text" id="host" name="host" size="45" class="validate[required]" value="<?= $database[0]; ?>"></td>
			</tr>
			<tr>
				<th>User:</th>
				<td><input type="text" id="user" name="user" size="45" class="validate[required]" value="<?= $database[1]; ?>"></td>
			</tr>
			<tr>
				<th>Password:</th>
				<td><input type="password" id="password" name="password" size="45" class="validate[required]" value="<?= $database[2]; ?>"></td>
			</tr>
			<tr>
				<th>Database:</th>
				<td><input type="text" id="database" name="database" size="45" class="validate[required,custom[dbname]]" value="<?= $database[3]; ?>"></td>
			</tr>
			<tr>
				<th>&nbsp;</th>
