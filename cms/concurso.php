...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	if(isset($_POST['edsList']))
	{
		...
		if($db->update('configuracion', 'eds="'.implode("\n", $eds).'"', '1=1 LIMIT 1'))
	...
	elseif(isset($_POST['form']) && $_POST['form']=='update')
	{
		...
		if($db->update('concurso', $values, 'con_pk="'.$_POST['con_pk'].'"'))
			echo "<script type=\"text/javascript\" language=\"javascript\"> alert('La información ha sido actualizada.') </script>";
		else
			echo "<script type=\"text/javascript\" language=\"javascript\"> alert('No fue posible actualizar toda la información.') </script>";
	}
...
$rowCon = $db->fetch_all($db->select('*', 'concurso ORDER BY fecha DESC'));
...
<header>
	<h1>Concurso - Pozo disponible: $<?= number_format($pozo, 0, '', '.'); ?></h1>
</header> <?
if($_SERVER['REQUEST_METHOD']=='GET' && isset($_GET['interfaz']))
{
	$rowPk = $db->fetch_array($db->select('*', 'concurso', 'con_pk="'.$_GET['con_pk'].'"')); ?>
...
<div class="contentPane">
	...
	<table width="100%" cellspacing="0" cellpadding="0" class="dataTable">
		<tr>
			...
			<td style="text-align: center;" width="60"><strong>Acciones</strong></td>
		</tr> <?
		if(count($rowCon)>0)
		{
			foreach($rowCon as $r)
		...
		{ ?>
			<tr><td colspan="5"><div align="center">No se han registrado solicitudes de inscripción.</div></td></tr> <?
		} ?>
	</table>
</div>
<div class="clear10"></div>
	...
	<form name="edsList" action="<?= $_SERVER['PHP_SELF']; ?>" method="post">
		<table class="formTable">
			<tr>
				...
				<td>
					<textarea id="eds" name="eds" cols="70" rows="8"><?= $rowConf['eds']; ?></textarea>
				</td>
			</tr>
			<tr>
				<td></td>
...
<div class="contentPane">
	...
	<table width="100%" cellspacing="0" cellpadding="0" class="dataTable">
		<tr>
			...
			<th style="text-align: center;"><strong>Inscrita por</strong></th>
			<td style="text-align: center;"><strong>Correo</strong></td>
