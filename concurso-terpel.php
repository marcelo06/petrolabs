...
if($_SERVER['REQUEST_METHOD']=='POST')
{
	...
	if($_POST['form']=='contest')
	{
		...
		{
			if($db->insert('concurso', 'tipo, eds, nombre, correo, ventas, estado', "'v', '".$_POST['eds']."', '".$_POST['nombre']."', '".$_POST['correo']."', '".$_POST['ventas']."', 'p'"))
			{
				...
				$body = '<p>Este es un mensaje enviado a trav&eacute;s del formulario de solicitud de premio del sitio web.</p><table border="0">'.
					'<tr><td><strong>Concurso:</strong></td><td>Terpel</td></tr>'.
					'<tr><td><strong>EDS preinscrita:</strong></td><td>'.$_POST['eds'].'</td></tr>'.
	...
	{
		if($db->insert('concurso', 'tipo, eds, nombre, correo, telefono, ventas, estado', "'i', '".$_POST['ieds']."', '".$_POST['inombre']."', '".$_POST['icorreo']."', '".$_POST['itelefono']."', '0', 'p'"))
		{
...
$rowConf = $db->fetch_array($db->select('eds', 'configuracion LIMIT 0,1'));
$rowCon = $db->fetch_all($db->select('*', 'concurso', 'tipo="v" AND estado!="r" ORDER BY fecha'));
...
<div class="whiteBg">
	<section class="centerContent">
		...
		<form id="inscription" name="inscription" method="post" action="<? $_SERVER['PHP_SELF']; ?>" class="pageContact">
			...
			<table class="contactTable" cellpadding="0" cellspacing="10">
				...
				<tr>
					<td>&nbsp;</td>
					<td> <?
						if(isset($msgInscription))
						{ ?>
							<script type="text/javascript"> alert('<?= $msgInscription; ?>'); </script>
							<span class="req"><?= $msgInscription; ?></span> <?
						} ?>
					</td>
				</tr>
			</table>
		...
		<form id="contest" name="contest" method="post" action="<? $_SERVER['PHP_SELF']; ?>" class="pageContact">
			...
			<table class="contactTable" cellpadding="0" cellspacing="10">
				<tr>
					<td><span class="req">*</span> EDS preinscrita:</td>
					<td>
